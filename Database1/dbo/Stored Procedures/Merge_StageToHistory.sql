...
CREATE PROCEDURE [dbo].[Merge_StageToHistory]
...
--Nedenst�ende Cursor benyttes til at lave en temp tabel, som indeholder alle kolonner med b�de Target_ og Source_ pr�fiks, s�ledes at vi kan smide vores output ned i denne og senere inds�tte det i vores history tabel.
...
OPEN PrimaryKey_column_cursor
	...
	BEGIN
		...
		IF (@SQL_ColumnString_UpdateSet <> '')
				...
				IF @DataType not in ('image','xml','ntext','text','geography','geometry')
	...
	WHEN NOT MATCHED BY SOURCE AND Target.HistoryValidDateTimeTo = ''9999-12-31'' THEN
	UPDATE SET 
	    Target.[HistoryDeletedAtSource] = 1,
		Target.[HistoryCurrentRec] = 0,
		Target.[HistoryValidDateTimeTo] = dateadd(NANOSECOND,-100, CAST(''' + @StartTime +''' as datetime2(7))),
		Target.[HistoryModifiedDate] = ''' + @StartTime +''',
		Target.[HistoryModifiedLogID] = '''+@LogID+'''	
...
/*The Deleted rows is set not to be deleted, because the deleted row is inserted below*/
Update ['+@DestinationDatabase+'].['+ @DestinationSchema +'].['+ @TableName + ']
SET [HistoryDeletedAtSource] = 0
WHERE [HistoryModifiedLogID] = '''+@LogID+''' AND HistoryCurrentRec = 0 AND HistoryDeletedAtSource = 1;
...
/*Insert of the deleted rows, therefore the HistoryValidDateTimeTo is 1 nanosecond less then HistoryValidDateTimeFrom, so it never can be selected*/
INSERT INTO ['+@DestinationDatabase+'].['+ @DestinationSchema +'].['+ @TableName + ']
('+@SQL_SelectColumnString_FromTemp+')
SELECT '+@SQL_ColumnString_InsertValues_OnDeleted +'
      ,'''+@LogID+''' --Inds�t LogID
      ,'''+@StartTime+'''
      ,dateadd(NANOSECOND,-100, CAST(''' + @StartTime +''' as datetime2(7)))
      ,NULL
      ,NULL
      ,0 --It is a deleted row, so it is not a current row
      ,1 --It is a deleted row, so it is a deleted row
  FROM [history_temp].['+ @Schema +'_'+ @TableName + ']
  where [Target_HistoryDeletedAtSource] = 1 AND [Action] = ''UPDATE'';
  '
	;
	PRINT @SQL;
	EXEC(@SQL);
