CREATE PROCEDURE [inf].[Stage_InfrastructureDatelag] 
AS
...
adresse_kvhx,
 TDC_Fiber_Enabled =
	...
	, HistoryValidDateTimeTo = ValidToDatetime
	, HistoryCurrentRec = IsCurrent
	, HistoryModifiedDate = IsDeleted
...
--	, ChurnLag =  LAG(MovementType,1,0) OVER (PARTITION BY Adresse_kvhx ORDER BY HistoryValidDateTimeFrom asc)
	...
	, HistoryValidDateTimeFrom 
	, HistoryValidDateTimeFromlag = LAG(HistoryValidDateTimeFrom, 1, 0) OVER (PARTITION BY Adresse_kvhx ORDER BY historyvaliddatetimefrom)
...
SELECT
	BK_TDC_KVHX = CAST(Adresse_Kvhx as varchar(19))
	...
	, BK_DateFromLag = Cast(HistoryValidDateTimeFromlag as date)
