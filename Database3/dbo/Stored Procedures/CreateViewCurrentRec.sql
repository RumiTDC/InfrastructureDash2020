...
Declare @CreateViewOnDB NVARCHAR(250) = Concat(@DestinationDatabase,'.sys.sp_executesql')
...
FOR
    ...
    SELECT
		...
		,CHAR(13) + CHAR(10)
		,'FROM '
		,'[', @DestinationDatabase,']','.[', [TABLE_SCHEMA], ']', '.[',[TABLE_NAME],']'
		,CHAR(13) + CHAR(10)
		,'WHERE 1 = 1'
		,CHAR(13) + CHAR(10)
		...
		,[COLUMN_NAME] = REPLACE(
		 ...
		 SELECT
			   ...
			   WHERE 1 = 1
				    ...
				    AND [C2].[COLUMN_NAME] NOT IN('HistoryLogID','HistoryValidDateTimeFrom','HistoryValidDateTimeTo','HistoryModifiedLogID','HistoryModifiedDate','HistoryCurrentRec','HistoryDeletedAtSource') FOR
			   XML PATH(''),TYPE).value
...
PRINT (@CRETAE_VIEW_QUERY)
	  ...
	  Select @CRETAE_VIEW_QUERY EXEC @CreateViewOnDB @CRETAE_VIEW_QUERY;	   -- This one creates the views		 
	   FETCH NEXT FROM crsMyTblParams INTO @DROP_VIEW_QUERY
								   ,@CRETAE_VIEW_QUERY;
    END;
